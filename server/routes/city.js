...
router.get('/', (req, res, next) => {
  ...
  CityCtrl.findAll(offset, limit, (err, rows) => {
    if (err) {
      console.log(err);
...
router.get('/:id', (req, res, next) => {
  const { id } = req.params;
  CityCtrl.findOneById(id, (err, data) => {
    if (err) {
      console.log(err);
      return res.status(500).send(err);
    }
...
router.put('/:id', (req, res, next) => {
  const { id } = req.params;
  const data = req.body;
  CityCtrl.updateOneById(id, data, (err, ret) => {
    if (err) {
      console.log(err);
      return res.status(500).send(err);
    }
...
router.delete('/:id', (req, res, next) => {
  const { id } = req.params;
  CityCtrl.deleteOneById(id, (err, ret) => {
    if (err) {
      console.log(err);
      return res.status(500).send(err);
    }
