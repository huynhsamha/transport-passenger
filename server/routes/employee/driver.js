...
router.get('/', (req, res, next) => {
  ...
  DriverCtrl.findAll(offset, limit, (err, rows) => {
    if (err) {
      console.log(err);
...
router.post('/', (req, res, next) => {
  const data = req.body;
  DriverCtrl.insert(data, (err, ret) => {
    if (err) {
      console.log(err);
      return res.status(500).send(err);
    }
...
router.get('/:id', (req, res, next) => {
  const { id } = req.params;
  DriverCtrl.findOneById(id, (err, data) => {
    if (err) {
      console.log(err);
      return res.status(500).send(err);
    }
...
router.put('/:id', (req, res, next) => {
  const { id } = req.params;
  const data = req.body;
  DriverCtrl.updateOneById(id, data, (err, ret) => {
    if (err) {
      console.log(err);
      return res.status(500).send(err);
    }
...
router.delete('/:id', (req, res, next) => {
  const { id } = req.params;
  DriverCtrl.deleteOneById(id, (err, ret) => {
    if (err) {
      console.log(err);
      return res.status(500).send(err);
    }
