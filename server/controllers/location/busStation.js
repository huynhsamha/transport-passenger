import { BusStation, Location } from '../../models';
...
const findOneById = (req, res, next) => {
  const { id } = req.params;
  BusStation.findById(id, { include: [{ model: Location, as: 'information' }] })
    .then((data) => {
      if (!data)
...
const updateOneById = async (req, res, next) => {
  ...
  try {
    let data = await BusStation.findById(id);
    if (!data) {
      return res.status(404).send({ message: 'Data not found' });
    }
...
const deleteOneById = async (req, res, next) => {
  const { id } = req.params;
  try {
    const data = await BusStation.findById(id);
    if (!data) {
      return res.status(404).send({ message: 'Data not found' });
    }
    await data.destroy();
